...
for i, url in enumerate(urls):
    ...
    subprocess.run(["python3", "-m", "epubcrush",
                    "--fast", "--remaster", "--asciionly", filename])
    check(filename)
    remaster_size = os.stat(filename).st_size // 1000
    ...
    print(
        ...
        f"| {remaster_size:4}kB ({(remaster_size/original_size):3.0%}) |"
    )
